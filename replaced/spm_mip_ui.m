...
if ~isfield(defaults,'animal');
    ...
    DXYZ = [defaults.animal.DX defaults.animal.DY defaults.animal.DZ];
    CXYZ = [defaults.animal.CX defaults.animal.CY defaults.animal.CZ];
    scale = defaults.animal.scale;
end
...
%-----------------------------------------------------------------------
set(hX,'Units','Data')
if(~exist('scale')) scale = 1; end; % ANIMAL addition
if length(hX)==1
	tmp = get(varargin{3},'UserData');
	vx  = sqrt(sum(tmp.M(1:3,1:3).^2)) / scale;
	tmp = tmp.M\[xyz ; 1];
	tmp = tmp(1:2).*vx(1:2)';
	set(hX,'Position',[tmp(1), tmp(2), 1])
else
	set(hX(1),'Position',[ Po(1) + xyz(2) / scale, Po(2) + xyz(1) / scale, 0])
	set(hX(2),'Position',[ Po(1) + xyz(2) / scale, Po(3) - xyz(3) / scale, 0])
	set(hX(3),'Position',[ Po(4) + xyz(1) / scale, Po(3) - xyz(3) / scale, 0])
end
...
d = get(cO,'Position');
        ...
        % Scale for animal coords if required
        if(~exist('scale')) scale = 1;end
...
if strcmp(sMarker,'hX1r')
	if b2d
		...
		xyz = [(d(2) - Po(2))*scale; (d(1) - Po(1))*scale; MS.xyz(3)];
	end
elseif strcmp(sMarker,'hX2r')
	xyz = [MS.xyz(1); (d(1) - Po(1))*scale; (Po(3) - d(2))*scale];
elseif strcmp(sMarker,'hX3r')
	xyz = [(d(1) - Po(4))*scale; MS.xyz(2); (Po(3) - d(2))*scale];
else
...
if b2d
	vx  = sqrt(sum(MS.M(1:3,1:3).^2)) * scale;
	tmp = MS.M\[xyz ; 1];
	tmp = tmp(1:2).*vx(1:2)';
	set(MS.hX,'Position',[tmp(1), tmp(2), 1],'Visible','on')
else
	set(MS.hX(1),'Position',[ Po(1) + xyz(2)/scale, Po(2) + xyz(1)/scale, 0])
	set(MS.hX(2),'Position',[ Po(1) + xyz(2)/scale, Po(3) - xyz(3)/scale, 0])
	set(MS.hX(3),'Position',[ Po(4) + xyz(1)/scale, Po(3) - xyz(3)/scale, 0])
end
