...
if strcmp(op,'zoom_in'),
	...
	if op==1,
		...
		pos = st.Space\[pos ; 1];
		pos = pos(1:3)';
		if     op == 2, st.bb = [pos-10*vx ; pos+10*vx] ; spm_orthviews('resolution',0.125);
		elseif op == 3, st.bb = [pos-5*vx ; pos+5*vx] ; spm_orthviews('resolution',0.0625);
		elseif op == 4, st.bb = [pos-1*vx ; pos+1*vx] ; spm_orthviews('resolution',0.03125);
		elseif op == 5, st.bb = [pos-0.4*vx ; pos+0.4*vx] ; spm_orthviews('resolution',0.015625);
        else            st.bb = [pos- 0.2*vx ; pos+ 0.2*vx] ; spm_orthviews('resolution',.00728125);
		end;
	end;
	return;
end;
