...
% $Id: spm_list.m 1079 2008-01-11 11:05:48Z guillaume $
...
if length(varargin) > 3
    ...
    % check if animal toolbox is being used
    if(~isfield(defaults,'animal'))
        Dis    = 8;
    else
        try % just in case defaults.animal.maximadistance hasn't been defined
            Dis = defaults.animal.maximadistance;
        catch
            disp('Warning: defaults.animal.maximadistance undefined. Using 8mm.');
            Dis = 8;
        end
    end
end
...
TabDat.hdr = {	'set',		'c';...
    ...
    % If using animal toolbox, load the format string for voxels to display
    % appropriate precision
    
    if(~isfield(defaults,'animal'))
        strVox = '%3.0f %3.0f %3.0f';
    else
        if(~isfield(defaults.animal,'voxelformat'))
            strVox = '%2.1f %2.1f %2.1f';
        else
            strVox = [defaults.animal.voxelformat ' ' defaults.animal.voxelformat ' ' defaults.animal.voxelformat];
        end
    end
...
TabDat.ftr{7} = ...
	['FWHM = ' sprintf('%0.3f ', FWmm) units{:} '; ' ... % changed %0.1 to %0.3 for more precision, animal
